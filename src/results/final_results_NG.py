...
for loss_type in loss_types:
    dirc = '../../output/20_newsgroups/ath.v.all/'
    ...
    for file_num, file in list(enumerate(files, start=1)):  # file = trails_1
        ...
        #print alpha_file
        file_t = sorted(
            alpha_file, key=lambda x: float(x.split("_")[1].split(".csv")[0]) ** -1)
        counter = 0
    ...
    for x in range(0, 58, 5):
        for y in range(0, 4):
            ...
            while lower <= upper:
                #print "y=",y
                measure_current = total[lower+y*58]
                current_cost = total_cost[y*58+lower]
            ...
            out = 'test_results_20NG/' + check(y) + str(x) + ".jpg"  
            print out
            P.savefig(out)  
            P.close(out)
