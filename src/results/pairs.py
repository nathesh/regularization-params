    ...
    for measure_1 in Measure:
        ...
        for measure_2 in Measure:
            if measure_1 != measure_2:
                ...
                if (mean_1 > mean_2) and (exp_1 > exp_2) and (main_mean_1> mean_1 and main_mean_2>mean_2):
...
def plot(pairs, Measures, Costs, num,loss_type,max_cost):
    ...
    for n,pair in list(enumerate(pairs)):
        ...
        sns.kdeplot(
            first_cost[:,1], gridsize=50, kernel='cos', ax=axf[n][2],label="step")
        sns.kdeplot(
            second_cost[:,1], gridsize=50, kernel='cos', ax=axf[n][3],label="step")
        axf[n][
        ...
        text = '$\hat{\mu}=%.2f$,$\mu=%.2f$\n(%.2f,%.2f)\n $\ \mathcal{L}_{e}=$%.2f' % (cv_first[num],
                float(np.mean(first_measure)), lower_percentile_first, upper_percentile_first, np.sum(first_cost[:,1]/max_cost[num][1]))
        props = dict(
        ...
        text = '$\hat{\mu}=%.2f$,$\mu=%.2f$\n(%.2f,%.2f)\n $\ \mathcal{L}_{e}=$%.2f' % (cv_second[num],
                float(np.mean(second_measure)), lower_percentile_second, upper_percentile_second, np.sum(second_cost[:,1]/max_cost[num][1])) 
        props = dict(
                boxstyle='round', facecolor='wheat', alpha=0.5)
