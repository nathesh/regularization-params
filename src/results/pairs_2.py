...
def get_values(loss_type_input):
    ...
    file_cost_open =  open(loss_types[0] + 'NG_cost.csv','r')
    ...
    for loss_type in loss_types:
        ...
        for path in files:  # each file with an alpha value
            ...
            cv = crossv.get_NG(float(alpha), loss_type)
...
def plot(pairs, Measures, Costs, num,loss_type,max_cost):
    ...
    for n,pair in list(enumerate(pairs)):
        ...
        C_2=float(pair[1])
        cv_first = crossv.get_NG(C_1,loss_type)
        cv_second = crossv.get_NG(C_2,loss_type)
        ...
        axf[n][1].text(0.95, 0.95, text, transform=axf[n][1].transAxes, fontsize=10,
                               verticalalignment='top', horizontalalignment='right', bbox=props)
        legend()
    out = "test_results_20NG/" + loss_type + '_' + get_num(num) + ".jpg"
    print out
    P.savefig(out)
    P.close(out)
